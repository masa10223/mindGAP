...
class DataProcessor:
    ...
    def load_excel_data(self, column: str = "IPS-22", 
        ...
        """
        if filepath is None:
            # 新しい構造：各質問票ごとのxlsxファイルを探す
            questionnaire_file = self.original_dir / f"{column}.xlsx"
            if questionnaire_file.exists():
                filepath = questionnaire_file
            else:
                # フォールバック：従来のdata.xlsxを探す
                ...
                if data_xlsx.exists():
                    ...
                    raise FileNotFoundError(f"Neither {column}.xlsx nor data.xlsx/data2.xlsx found in {self.original_dir}")
        
        # 新しい構造の場合：単一シートを読み込み
        if filepath.name.endswith(f"{column}.xlsx"):
            # 最初の行は質問文、2行目は選択肢説明、3行目以降が回答データ
            df = pd.read_excel(filepath, sheet_name=0, header=2, engine="openpyxl")
            # 列名を列番号に変更
            df.columns = range(len(df.columns))
            return df
        else:
            # 従来の構造：全てのシートを辞書として読み込み
    ...
    def process_questionnaire_data(self, 
        ...
        # HQ-25+4の場合は年代別に列を選択
        if questionnaire_type == "HQ-25+4" and time_period and time_period in hq25_time_ranges:
            start_col, end_col = hq25_time_ranges[time_period]
            raw_data = raw_data.iloc[:, start_col:end_col].copy()
            # 列名を列番号に変更
        ...
        if column_indices is not None:
            # 特定の列インデックスを選択
            valid_indices = [i for i in column_indices if 0 <= i < len(processed_data.columns)]
            if not valid_indices:
                raise ValueError(f"No valid column indices found. Available columns: 0-{len(processed_data.columns)-1}")
            processed_data = processed_data.iloc[:, valid_indices]
            # 列名を抽出した列番号に変更
            processed_data.columns = range(len(processed_data.columns))
        elif column_range is not None:
            # 列範囲を選択
